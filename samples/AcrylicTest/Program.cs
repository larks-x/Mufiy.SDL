...
unsafe
{
    ...
    var window = SDL_CreateWindow("Acrylic Test", 800, 600, SDL_WindowFlags.Borderless | SDL_WindowFlags.Resizable | SDL_WindowFlags.HighPixelDensity);
    if (window.IsNull)
    {
    ...
    if (hwnd != 0)
    {
        ...
        if (hr != 0)
        {
            Win32.DWM_BLURBEHIND bb = new() { dwFlags = 1, fEnable = 1 };
            Win32.DwmEnableBlurBehindWindow(hwnd, ref bb);
        }
...
static class Win32
{
    ...
    [DllImport("dwmapi.dll")]
    public static extern int DwmEnableBlurBehindWindow(nint hwnd, ref DWM_BLURBEHIND pBlurBehind);
}
